...
clawreform-cli            CLI interface, daemon auto-detect, MCP server
    |
clawreform-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
clawreform-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
clawreform-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- clawreform-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- clawreform-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- clawreform-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- clawreform-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- clawreform-skills     60 bundled skills, FangHub marketplace, ClawHub client
    |
clawreform-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
clawreform-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
| **clawreform-skills** | Skill system for pluggable tool bundles. 60 bundled skills compiled via `include_str!()`. Skills are `skill.toml` + Python/WASM/Node.js/PromptOnly code. `SkillManifest` defines metadata, runtime config, provided tools, and requirements. `SkillRegistry` manages installed and bundled skills. `FangHubClient` connects to FangHub marketplace. `ClawHubClient` connects to clawhub.ai for cross-ecosystem skill discovery. `SKILL.md` parser for OpenClaw compatibility (YAML frontmatter + Markdown body). `SkillVerifier` with SHA256 verification. Prompt injection scanner (`scan_prompt_content()`) detects override attempts, data exfiltration, and shell references. |
...
The memory substrate (`clawreform-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`clawreform-runtime`) provides a unified interface for all LLM providers:
...
The `ModelCatalog` (`clawreform-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
`taint.rs` in `clawreform-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`clawreform-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`clawreform-skills`) provides 60 bundled skills and supports external skill installation.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| clawreform-runtime |  | clawreform-    |  | open-  |  | clawreform- |
