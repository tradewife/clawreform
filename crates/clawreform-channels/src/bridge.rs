//! Channel bridge — connects channel adapters to the ClawReform kernel.
//!
//! Defines `ChannelBridgeHandle` (implemented by clawreform-api on the kernel) and
...
///
/// Defined here to avoid circular deps (clawreform-channels can't depend on clawreform-kernel).
/// Implemented in clawreform-api on the actual kernel.
#[async_trait]
...
async fn dispatch_message(
    ...
    if router.has_broadcast(&message.sender.platform_id) {
        ...
        if !targets.is_empty() {
            ...
            match strategy {
                clawreform_types::config::BroadcastStrategy::Parallel => {
                ...
                clawreform_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
    ...
    let agent_id = router.resolve(
        ...
        message.sender.clawreform_user.as_deref(),
    );
    ...
    match name {
        ...
        "new" => {
            ...
            let agent_id = router.resolve(
                ...
                &sender.platform_id,
                sender.clawreform_user.as_deref(),
            );
            match agent_id {
                Some(aid) => handle
                    .reset_session(aid)
