...
3. starts each assigned CLI inside a detached `screen` session
...
That starts the agents and opens visible `kitty` windows attached to each live session.
...
Use this if the `screen` sessions already exist and you only want the windows back.
...
- attaches to one `screen` session
- stays open at a shell prompt after the session exits
- prints the log path when the attached session ends
