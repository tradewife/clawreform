...
1. **Add token estimation & context guard** (`crates/clawreform-runtime/src/compactor.rs`)
...
2. **Add in-loop token guard** (`crates/clawreform-runtime/src/agent_loop.rs`)
...
3. **Filter tools by profile in kernel** (`crates/clawreform-kernel/src/kernel.rs`)
...
4. **Raise default token quota** (`crates/clawreform-types/src/agent.rs`)
   - Change `max_llm_tokens_per_hour` from 100_000 to 1_000_000
   - 100K is too low — a single system prompt is 30-40K tokens

5. **Token-based compaction trigger** (`crates/clawreform-kernel/src/kernel.rs`)
...
6. **Compact system prompt injections** (`crates/clawreform-kernel/src/kernel.rs`)
...
**Problem:** Desktop app may show Tauri default icons. Branding assets exist at `~/Downloads/clawreform/output/` but aren't installed.

**What to do:**

1. Generate all required icon sizes from source PNG (`clawreform-logo-transparent.png`, 2000x2000)
2. Place into `crates/clawreform-desktop/icons/`:
   - `icon.png` (1024x1024)
   - `icon.ico` (multi-size: 256, 128, 64, 48, 32, 16)
   - `32x32.png`
   - `128x128.png`
   - `128x128@2x.png` (256x256)
3. Replace web UI logo at `crates/clawreform-api/static/logo.png`
4. Update favicon if one exists

**Assets available:**
- `clawreform-logo-transparent.png` (328KB, 2000x2000) — primary source
- `clawreform-logo-black-bg.png` (312KB) — for dark contexts
- `clawreform-vector-transparent.svg` (293KB) — scalable vector
...
**Status: COMPLETE** — Generated Ed25519 signing keypair via `cargo tauri signer generate --ci`. Public key installed in `tauri.conf.json`. Private key at `~/.tauri/clawreform.key`. Set `TAURI_SIGNING_PRIVATE_KEY_PATH` in CI secrets.
...
**Files:** `crates/clawreform-api/static/js/pages/chat.js`, `crates/clawreform-runtime/src/tool_runner.rs`
...
**Files:** `crates/clawreform-api/static/js/pages/skills.js`, `crates/clawreform-api/src/routes.rs`
...
**Problem:** `clawreform.sh` domain isn't set up. Users can't do `curl -sSf https://clawreform.sh | sh`.

**What to do:**
1. Set up GitHub Pages or Cloudflare Worker for clawreform.sh
...
**Done when:**
- `curl -sSf https://clawreform.sh | sh` installs the latest release
- `irm https://clawreform.sh/install.ps1 | iex` works on Windows PowerShell
...
**Status: COMPLETE** — Created `sdk/javascript/` (@clawreform/sdk) with full REST client: agent CRUD, streaming via SSE, sessions, workflows, skills, channels, memory KV, triggers, schedules + TypeScript declarations. Created `sdk/python/clawreform_client.py` (zero-dependency stdlib urllib) with same coverage. Both include basic + streaming examples. Python `setup.py` for pip install.
...
1. Create `sdks/javascript/` — thin wrapper around REST API
   ...
   - Publish to npm as `@clawreform/sdk`
2. Create `sdks/python/` — thin wrapper with httpx
   - Same operations
   - Publish to PyPI as `clawreform`
3. Include usage examples in README

**Done when:**
- `npm install @clawreform/sdk` works
- `pip install clawreform` works
...
1. Add `/api/metrics` endpoint with Prometheus format
   - `clawreform_agents_active` gauge
   - `clawreform_messages_total` counter (by agent, by channel)
   - `clawreform_tokens_total` counter (by provider, by model)
   - `clawreform_request_duration_seconds` histogram
   - `clawreform_tool_calls_total` counter (by tool name)
   - `clawreform_errors_total` counter (by type)
2. Optional: OTLP export for tracing spans

**Files:** `crates/clawreform-api/src/routes.rs`, new `metrics.rs` module
...
**Files:** `crates/clawreform-kernel/src/kernel.rs`, `routes.rs`, `ws.rs`, `index_body.html`
...
**What to do:**
1. Watch `~/.clawreform/config.toml` for changes (notify crate)
2. On change: re-parse, diff, apply only changed sections
3. Log what was reloaded
4. UI notification: "Config reloaded"

**Files:** `crates/clawreform-api/src/server.rs`, `crates/clawreform-types/src/config.rs`
...
**Status: ALL CODE COMPLETE** — All 18 code items done. 1751 tests passing. Production audit completed: 2 critical bugs fixed (API delete alias, config/set route), CSP hardened (Tauri + middleware), Tauri signing key installed. Remaining for release: tag v0.1.0, build release artifacts, set up clawreform.sh domain.
...
Sprint 2: 4/5 COMPLETE
  ...
  2.4 Install script domain ........ PENDING (infra: set up clawreform.sh domain)
