"""
clawREFORM by aegntic.ai Python SDK — helper library for writing Python agents.
...
Or for simple scripts without the decorator pattern:
    ...
    from clawreform_sdk import read_input, respond
...
def read_input() -> Dict[str, Any]:
    """Read the input JSON from stdin (sent by the clawREFORM by aegntic.ai kernel)."""
    line = sys.stdin.readline().strip()
    if not line:
        # Fallback: check environment variables
        agent_id = os.environ.get("CLAWREFORM_AGENT_ID", "")
        message = os.environ.get("CLAWREFORM_MESSAGE", "")
        return {
            "type": "message",
            "agent_id": agent_id,
            "message": message,
...
def respond(text: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """Send a response back to the clawREFORM by aegntic.ai kernel via stdout."""
...
def log(message: str, level: str = "info") -> None:
    """Log a message to stderr (visible in clawREFORM by aegntic.ai daemon logs)."""
...
if __name__ == "__main__":
    print("clawREFORM by aegntic.ai Python SDK")
    print("====================")
    print()
    print("Import this module in your agent scripts:")
    print()
    print("  from clawreform_sdk import Agent")
    print()
    print("  agent = Agent()")
    print()
