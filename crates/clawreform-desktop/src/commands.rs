...
/// `~/.clawreform/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = clawreform_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.clawreform/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = clawreform_home().join("skills");
...
/// Open the ClawReform config directory (`~/.clawreform/`) in the OS file manager.
#[tauri::command]
pub fn open_config_dir() -> Result<(), String> {
    let dir = clawreform_home();
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create config dir: {e}"))?;
    open::that(&dir).map_err(|e| format!("Failed to open directory: {e}"))
}

/// Open the ClawReform logs directory (`~/.clawreform/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = clawreform_home().join("logs");
