...
//!
//! Allows any OpenAI-compatible client library to talk to ClawReform agents.
//! The `model` field resolves to an agent (by name, UUID, or `clawreform:<name>`),
...
use clawreform_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "clawreform:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("clawreform:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("clawreform:{}", e.name),
            object: "model",
            created,
            owned_by: "clawreform".to_string(),
        })
        .collect();
