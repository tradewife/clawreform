// ClawReform Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async startQR() {
      ...
      try {
        var result = await ClawReformAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          ClawReformToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await ClawReformAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            ClawReformToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await ClawReformAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await ClawReformAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            ClawReformToast.success(this.setupModal.display_name + ' activated!');
          } else {
            ClawReformToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          ClawReformToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
    ...
    async removeChannel() {
      ...
      var self = this;
      ClawReformToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await ClawReformAPI.delete('/api/channels/' + name + '/configure');
          ClawReformToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await ClawReformAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          ClawReformToast.success(result.message);
        } else {
          ClawReformToast.error(result.message);
        }
      } catch(e) {
        ClawReformToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        ClawReformToast.success('Copied to clipboard');
      } catch(e) {
        ClawReformToast.error('Copy failed');
      }
    },
