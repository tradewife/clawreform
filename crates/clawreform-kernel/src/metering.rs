...
use clawreform_memory::usage::{ModelUsage, UsageRecord, UsageStore, UsageSummary};
...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> ClawReformResult<()> {
        // Hourly check
    ...
    pub fn check_global_budget(
        &self,
        budget: &clawreform_types::config::BudgetConfig,
    ) -> ClawReformResult<()> {
    ...
    pub fn budget_status(&self, budget: &clawreform_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> ClawReformResult<UsageSummary> {
        self.store.query_summary(agent_id)
    }

    /// Get usage grouped by model.
    pub fn get_by_model(&self) -> ClawReformResult<Vec<ModelUsage>> {
    ...
    pub fn estimate_cost_with_catalog(
        catalog: &clawreform_runtime::model_catalog::ModelCatalog,
        model: &str,
        input_tokens: u64,
        output_tokens: u64,
    ) -> f64 {
    ...
    pub fn cleanup(&self, days: u32) -> ClawReformResult<usize> {
