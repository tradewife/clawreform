...
pub struct AuthManager {
    /// Known users by their ClawReform user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the ClawReform UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> ClawReformResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| ClawReformError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(ClawReformError::AuthDenied(format!(
