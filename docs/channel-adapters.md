...
All channel configurations live in `~/.clawreform/config.toml` under the `[channels]` section. Each channel is a subsection:
...
The `formatter` module (`clawreform-channels/src/formatter.rs`) converts Markdown output from the LLM into platform-native formats:
...
The `ChannelRateLimiter` (`clawreform-channels/src/rate_limiter.rs`) uses a `DashMap` to track per-user message counts. When `rate_limit_per_user` is set on a channel's overrides, the limiter enforces a sliding-window cap of N messages per minute. Excess messages receive a polite rejection.
...
user_id = "@clawreform-bot:matrix.org"
default_agent = "assistant"
```
...
To add support for a new messaging platform, implement the `ChannelAdapter` trait. The trait is defined in `crates/clawreform-channels/src/types.rs`.
...
Create `crates/clawreform-channels/src/myplatform.rs`:
...
In `crates/clawreform-channels/src/lib.rs`:
...
In `crates/clawreform-api/src/channel_bridge.rs`, add initialization logic for your adapter alongside the existing adapters.

### 4. Add Config Support

In `clawreform-types`, add a config struct:
...
In `crates/clawreform-cli/src/main.rs`, add a case to `cmd_channel_setup` with step-by-step instructions for your platform.
