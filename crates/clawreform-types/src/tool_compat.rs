//! Shared tool name mappings between OpenClaw and ClawReform.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize OpenClaw tool names into ClawReform equivalents.

/// Map an OpenClaw tool name to its ClawReform equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an ClawReform tool name — check with [`is_known_clawreform_tool`]).
...
/// Check if a tool name is a known ClawReform built-in tool.
pub fn is_known_clawreform_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_is_known_clawreform_tool() {
        ...
        assert!(!is_known_clawreform_tool("Bash"));
    }
}
