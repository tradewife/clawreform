//! Minimal `.env` file loader/saver for `~/.clawreform/.env`.
//!
...
/// Return the path to `~/.clawreform/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".clawreform").join(".env"))
}

/// Load `~/.clawreform/.env` and `~/.clawreform/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.clawreform/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".clawreform").join("secrets.env"))
}
...
/// Upsert a key in `~/.clawreform/.env`.
///
...
/// Remove a key from `~/.clawreform/.env`.
///
...
/// List key names (without values) from `~/.clawreform/.env`.
#[allow(dead_code)]
