...
mod tests {
    ...
    fn resolve_client_ids_uses_defaults() {
        let config = clawreform_types::config::OAuthConfig::default();
        let ids = resolve_client_ids(&config);
        assert_eq!(ids["google"], "clawreform-google-client-id");
        assert_eq!(ids["github"], "clawreform-github-client-id");
    }

    #[test]
    fn resolve_client_ids_applies_overrides() {
        let config = clawreform_types::config::OAuthConfig {
        ...
        assert_eq!(ids["github"], "clawreform-github-client-id"); // default
        assert_eq!(ids["microsoft"], "my-msft-id");
        assert_eq!(ids["slack"], "clawreform-slack-client-id"); // default
    }
}
