...
use clawreform_types::agent::{AgentEntry, AgentId, SessionId};
use clawreform_types::error::{ClawReformError, ClawReformResult};
use clawreform_types::memory::{
...
impl MemorySubstrate {
    ...
    pub fn open(db_path: &Path, decay_rate: f32) -> ClawReformResult<Self> {
        let conn = Connection::open(db_path).map_err(|e| ClawReformError::Memory(e.to_string()))?;
        conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000;")
            .map_err(|e| ClawReformError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| ClawReformError::Memory(e.to_string()))?;
    ...
    pub fn open_in_memory(decay_rate: f32) -> ClawReformResult<Self> {
        let conn =
            Connection::open_in_memory().map_err(|e| ClawReformError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| ClawReformError::Memory(e.to_string()))?;
        let shared = Arc::new(Mutex::new(conn));
    ...
    pub fn structured_delete(&self, agent_id: AgentId, key: &str) -> ClawReformResult<()> {
    ...
    ) -> ClawReformResult<(Option<String>, Vec<clawreform_types::message::Message>)> {
    ...
    pub fn store_llm_summary(
        ...
        agent_id: AgentId,
        summary: &str,
        kept_messages: Vec<clawreform_types::message::Message>,
    ) -> ClawReformResult<()> {
        self.sessions
    ...
    pub fn append_canonical(
        &self,
        agent_id: AgentId,
        messages: &[clawreform_types::message::Message],
    ...
    pub fn load_paired_devices(&self) -> ClawReformResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| ClawReformError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Memory(e.to_string()))?;
    ...
    ) -> ClawReformResult<Vec<MemoryFragment>> {
        self.semantic
            .recall_with_embedding(query, limit, filter, query_embedding)
    }

    /// Update the embedding for an existing memory.
    pub fn update_embedding(&self, id: MemoryId, embedding: &[f32]) -> ClawReformResult<()> {
    ...
    pub async fn task_post(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let db = conn.lock().map_err(|e| ClawReformError::Internal(e.to_string()))?;
            db.execute(
    ...
    pub async fn task_claim(&self, agent_id: &str) -> ClawReformResult<Option<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
    ...
    pub async fn task_complete(&self, task_id: &str, result: &str) -> ClawReformResult<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if rows == 0 {
                return Err(ClawReformError::Internal(format!("Task not found: {task_id}")));
            }
            Ok(())
        })
        .await
    ...
    pub async fn task_list(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = db.prepare(sql).map_err(|e| ClawReformError::Memory(e.to_string()))?;
            ...
            for row in rows {
                tasks.push(row.map_err(|e| ClawReformError::Memory(e.to_string()))?);
            }
            Ok(tasks)
        })
        .await
...
impl Memory for MemorySubstrate {
    ...
    async fn delete(&self, agent_id: AgentId, key: &str) -> ClawReformResult<()> {
        let store = self.structured.clone();
        let key = key.to_string();
    ...
    async fn forget(&self, id: MemoryId) -> ClawReformResult<()> {
        let store = self.semantic.clone();
    ...
    async fn add_entity(&self, entity: Entity) -> ClawReformResult<String> {
    ...
    async fn add_relation(&self, relation: Relation) -> ClawReformResult<String> {
        let store = self.knowledge.clone();
    ...
    async fn query_graph(&self, pattern: GraphPattern) -> ClawReformResult<Vec<GraphMatch>> {
        let store = self.knowledge.clone();
    ...
    async fn consolidate(&self) -> ClawReformResult<ConsolidationReport> {
    ...
    async fn export(&self, format: ExportFormat) -> ClawReformResult<Vec<u8>> {
        let _ = format;
        Ok(Vec::new())
    }

    async fn import(&self, _data: &[u8], _format: ExportFormat) -> ClawReformResult<ImportReport> {
        Ok(ImportReport {
            entities_imported: 0,
