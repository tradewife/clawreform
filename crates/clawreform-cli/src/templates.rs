...
/// 2. `~/.clawreform/agents/` (installed templates)
/// 3. `CLAWREFORM_AGENTS_DIR` env var
pub fn discover_template_dirs() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        let agents = home.join(".clawreform").join("agents");
    ...
    if let Ok(env_dir) = std::env::var("CLAWREFORM_AGENTS_DIR") {
