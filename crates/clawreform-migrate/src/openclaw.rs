...
// ---------------------------------------------------------------------------
// ClawReform output types (TOML)
// ---------------------------------------------------------------------------

/// ClawReform config.toml structure for serialization.
#[derive(Serialize)]
struct ClawReformConfig {
    default_model: ClawReformModelConfig,
    memory: ClawReformMemorySection,
    network: ClawReformNetworkSection,
    #[serde(skip_serializing_if = "Option::is_none")]
    channels: Option<toml::Value>,
}

#[derive(Serialize)]
struct ClawReformModelConfig {
    provider: String,
    model: String,
    api_key_env: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Serialize)]
struct ClawReformMemorySection {
    decay_rate: f32,
}

#[derive(Serialize)]
struct ClawReformNetworkSection {
    listen_addr: String,
}
...
/// Map OpenClaw DM policy to ClawReform DM policy string.
...
/// Map OpenClaw group policy to ClawReform group policy string.
...
use clawreform_types::tool_compat::{is_known_clawreform_tool, map_tool_name};

/// Map OpenClaw tool profile to ClawReform capability tool list.
...
/// Map OpenClaw provider name to ClawReform provider name.
...
fn migrate_config_from_json(
    ...
    let of_config = ClawReformConfig {
        default_model: ClawReformModelConfig {
            provider,
            model,
            api_key_env,
        ...
        memory: ClawReformMemorySection { decay_rate: 0.05 },
        network: ClawReformNetworkSection {
...
fn migrate_channels_from_json(
    ...
    // --- BlueBubbles (skip — no ClawReform adapter) ---
    if oc_channels.bluebubbles.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: "bluebubbles".to_string(),
            reason: "No ClawReform adapter available — consider using the iMessage channel instead"
                .to_string(),
        });
    }
    ...
    for key in oc_channels.other.keys() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Channel,
            name: key.clone(),
            reason: format!("Unknown channel '{key}' — not mapped to any ClawReform adapter"),
        });
    }
            ...
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{id}': tool '{tool}' has no ClawReform equivalent and was skipped"
                    ));
                }
...
fn convert_agent_from_json(
    ...
    toml_str.push_str("author = \"clawreform\"\n");
...
fn report_skipped_features(root: &OpenClawRoot, source: &Path, report: &mut MigrationReport) {
    ...
    if root.cron.is_some() {
        report.skipped.push(SkippedItem {
            kind: ItemKind::Config,
            name: "cron".to_string(),
            reason: "Cron job scheduling not yet supported — use ClawReform's ScheduleMode::Periodic instead".to_string(),
        });
    }
        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `clawreform skill install`".to_string(),
                });
            }
        }
    }
    ...
    if source.join("memory-search").join("index.db").exists() {
        report.skipped.push(SkippedItem {
            ...
            reason: "SQLite vector index not portable — ClawReform will rebuild embeddings"
                .to_string(),
        });
    }
    ...
    if root.session.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason: "Session scope config differs — ClawReform uses per-agent sessions by default"
                .to_string(),
        });
    }
    ...
    if root.memory.is_some() {
        report.skipped.push(SkippedItem {
            ...
            reason:
                "Memory backend config not migrated — ClawReform uses SQLite with vector embeddings"
                    .to_string(),
        });
    }
}
        ...
        match *name {
            ...
            "bluebubbles" => {
                report.skipped.push(SkippedItem {
                    kind: ItemKind::Channel,
                    name: "bluebubbles".to_string(),
                    reason: "No ClawReform adapter available — consider using the iMessage channel instead".to_string(),
                });
            }
            _ => {}
        }
    ...
    for entry in entries {
        ...
        match convert_legacy_agent(&agent_yaml, &agent_name) {
            Ok((toml_str, unmapped_tools)) => {
                ...
                for tool in &unmapped_tools {
                    report.warnings.push(format!(
                        "Agent '{agent_name}': tool '{tool}' has no ClawReform equivalent and was skipped"
                    ));
                }
...
mod tests {
  ...
  agents: {
    ...
    #[test]
    fn test_is_known_clawreform_tool() {
        assert!(is_known_clawreform_tool("file_read"));
        assert!(is_known_clawreform_tool("shell_exec"));
        assert!(is_known_clawreform_tool("web_fetch"));
        assert!(!is_known_clawreform_tool("Read"));
        assert!(!is_known_clawreform_tool("unknown"));
    }
