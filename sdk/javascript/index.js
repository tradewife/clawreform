 ...
 *   const client = new ClawReform("http://localhost:3000");
 *
...
class ClawReformError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "ClawReformError";
    this.status = status;
    this.body = body;
  }
}
...
class ClawReform {
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new ClawReformError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new ClawReformError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { ClawReform: ClawReform, ClawReformError: ClawReformError };
