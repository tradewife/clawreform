...
**Fix**: Run `clawreform init` to create the default config:
```bash
clawreform init
```

This creates `~/.clawreform/config.toml` with sensible defaults.
...
```bash
RUST_LOG=clawreform_channels=debug clawreform start
```
...
**Check**:
1. TOML manifest is valid: `clawreform agent spawn --dry-run manifest.toml`
...
2. Set the model to `clawreform:agent-name` (e.g., `clawreform:coder`)
...
3. Try deleting `~/.clawreform/daemon.json` and restarting
...
If slower:
- Check database size (`~/.clawreform/data/clawreform.db`)
...
1. Add the channel config to `~/.clawreform/config.toml` under `[channels]`
...
```bash
# From source
cd clawreform && git pull && cargo install --path crates/clawreform-cli
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.clawreform/data/clawreform.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.clawreform/config.toml` (configuration)
- `~/.clawreform/data/clawreform.db` (all agent data, memory, sessions)
...
```toml
[dependencies]
clawreform-runtime = { path = "crates/clawreform-runtime" }
clawreform-memory = { path = "crates/clawreform-memory" }
```

The `clawreform-kernel` crate assembles everything, but you can use individual crates for custom integrations.
