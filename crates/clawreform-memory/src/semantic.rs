...
use clawreform_types::memory::{MemoryFilter, MemoryFragment, MemoryId, MemorySource};
...
impl SemanticStore {
    ...
    pub fn recall_with_embedding(
        ...
        for row_result in rows {
            let (
                ...
                access_count,
                embedding_bytes,
            ) = row_result.map_err(|e| ClawReformError::Memory(e.to_string()))?;
    ...
    pub fn forget(&self, id: MemoryId) -> ClawReformResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| ClawReformError::Internal(e.to_string()))?;
