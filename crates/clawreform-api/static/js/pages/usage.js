// ClawReform Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await ClawReformAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await ClawReformAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await ClawReformAPI.get('/api/usage/daily');
