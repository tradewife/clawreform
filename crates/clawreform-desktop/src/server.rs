...
//!
//! Boots the ClawReform kernel, binds to a random localhost port, and runs the
//! API server on a background thread with its own tokio runtime.

use clawreform_api::server::build_router;
...
impl ServerHandle {
    ...
    pub fn shutdown(mut self) {
        ...
        info!("ClawReform embedded server stopped");
    }
}
...
pub fn start_server() -> Result<ServerHandle, Box<dyn std::error::Error>> {
    ...
    let kernel = ClawReformKernel::boot(None)?;
    ...
    info!("ClawReform embedded server bound to http://127.0.0.1:{port}");

    let (shutdown_tx, shutdown_rx) = watch::channel(false);
    let kernel_clone = kernel.clone();

    let server_thread = std::thread::Builder::new()
        .name("clawreform-server".into())
        .spawn(move || {
...
async fn run_embedded_server(
    ...
    info!("ClawReform embedded server listening on http://{listen_addr}");
