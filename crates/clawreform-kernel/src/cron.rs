//! Cron job scheduler engine for the ClawReform kernel.
//!
...
///
/// The `CronJob` struct in `clawreform-types` is intentionally lean (no
...
impl CronScheduler {
    ...
    ///
    /// `home_dir` is the ClawReform data directory; jobs are persisted to
    ...
    pub fn load(&self) -> ClawReformResult<usize> {
        if !self.persist_path.exists() {
            return Ok(0);
        }
        let data = std::fs::read_to_string(&self.persist_path)
            .map_err(|e| ClawReformError::Internal(format!("Failed to read cron jobs: {e}")))?;
        let metas: Vec<JobMeta> = serde_json::from_str(&data)
            .map_err(|e| ClawReformError::Internal(format!("Failed to parse cron jobs: {e}")))?;
        let count = metas.len();
        for meta in metas {
    ...
    pub fn persist(&self) -> ClawReformResult<()> {
        ...
        std::fs::rename(&tmp_path, &self.persist_path).map_err(|e| {
            ClawReformError::Internal(format!("Failed to rename cron jobs file: {e}"))
        })?;
    ...
    pub fn add_job(&self, mut job: CronJob, one_shot: bool) -> ClawReformResult<CronJobId> {
        // Global limit
        let max_jobs = self.max_total_jobs.load(Ordering::Relaxed);
        if self.jobs.len() >= max_jobs {
            return Err(ClawReformError::Internal(format!(
        ...
        job.validate(agent_count)
            .map_err(ClawReformError::InvalidInput)?;
    ...
    pub fn remove_job(&self, id: CronJobId) -> ClawReformResult<CronJob> {
        self.jobs
            .remove(&id)
            .map(|(_, meta)| meta.job)
            .ok_or_else(|| ClawReformError::Internal(format!("Cron job {id} not found")))
    }

    /// Enable or disable a job. Re-enabling resets errors and recomputes
    /// `next_run`.
    pub fn set_enabled(&self, id: CronJobId, enabled: bool) -> ClawReformResult<()> {
...
mod tests {
    use super::*;
    use chrono::Duration;
    use clawreform_types::scheduler::{CronAction, CronDelivery};
    ...
    fn test_add_job_per_agent_limit() {
        // MAX_JOBS_PER_AGENT = 50 in clawreform-types
