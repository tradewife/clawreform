...
# --- Memory ---
[memory]
# sqlite_path = "~/.clawreform/data/clawreform.db"  # Auto-resolved if omitted
...
```toml
[memory]
# sqlite_path = "/custom/path/clawreform.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/clawreform.db`. |
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@clawreform:matrix.org"`). |
...
| `nick` | string | `"clawreform"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#clawreform", "#general"]`). |
...
| `nick` | string | `"clawreform"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"clawreform"` | Bot username in Mumble. |
