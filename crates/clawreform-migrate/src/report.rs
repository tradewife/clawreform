...
impl MigrationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str("1. Review imported agent manifests in `~/.clawreform/agents/`\n");
        out.push_str(
            "2. Review `~/.clawreform/secrets.env` — verify tokens were migrated correctly\n",
        );
        out.push_str("3. Set any remaining API keys referenced in `~/.clawreform/config.toml`\n");
        out.push_str("4. Start the daemon: `clawreform start`\n");
        out.push_str("5. Test your agents: `clawreform agent list`\n");
    ...
    pub fn print_summary(&self) {
        ...
        if !self.dry_run {
            println!("\n  Next steps:");
            println!("    clawreform start");
            println!("    clawreform agent list");
        }
    }
}
    ...
    fn test_report_with_items() {
        let report = MigrationReport {
            ...
            imported: vec![MigrateItem {
                ...
                destination: "~/.clawreform/agents/coder/agent.toml".to_string(),
            }],
