...
```python
#!/usr/bin/env python3
from clawreform_sdk import SkillHandler
...
```bash
clawreform skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.clawreform/skills/my-skill/`.

### From FangHub

```bash
clawreform skill install web-summarizer
```
...
```bash
clawreform skill install https://github.com/user/clawreform-skill-example.git
```
...
```bash
clawreform skill remove web-summarizer
```
...
```bash
clawreform skill search "web scraping"
```
...
```bash
clawreform skill install /path/to/skill-directory
```

Skills imported via `clawreform migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.
