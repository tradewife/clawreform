// ClawReform Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await ClawReformAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await ClawReformAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        ClawReformToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        ClawReformToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await ClawReformAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        ClawReformToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        ClawReformToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await ClawReformAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        ClawReformToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        ClawReformToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      ClawReformToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await ClawReformAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          ClawReformToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          ClawReformToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await ClawReformAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          ClawReformToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          ClawReformToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        ClawReformToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await ClawReformAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        ClawReformToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        ClawReformToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      ClawReformToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await ClawReformAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          ClawReformToast.success('Trigger deleted');
        } catch(e) {
          ClawReformToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
