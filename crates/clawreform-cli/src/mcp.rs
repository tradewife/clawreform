//! MCP (Model Context Protocol) server for ClawReform.
//!
//! Exposes running agents as MCP tools over JSON-RPC 2.0 stdio.
//! Each agent becomes a callable tool named `clawreform_agent_{name}`.
//!
...
impl McpBackend {
    ...
    fn resolve_tool_agent(&self, tool_name: &str) -> Option<String> {
        ...
        let agent_name_underscore = tool_name.strip_prefix("clawreform_agent_")?;
        for (id, name, _) in &agents {
...
mod tests {
    ...
    fn test_handle_initialize() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "clawreform");
    }
